...
class Decoder(nn.Module):
    ...
    def _process_inputs(self, decoder_input, subject_embedded, tgt_key_padding_mask=None):
    ...
    def single_step_decode(self, memory, subject_embedded, tgt_key_padding_mask=None):
        ...
        embedded, tgt_key_padding_mask = self._process_inputs(
            decoder_input, subject_embedded, tgt_key_padding_mask)
    ...
    def autoregressive_decode(self, memory, subject_embedded, target=None, teacher_forcing_ratio=0.5):
        ...
        for t in range(self.seq_length):
            ...
            embedded, _ = self._process_inputs(
                decoder_input, subject_embedded[:, t:t+1, :])
    ...
    def forward(self, memory, subject_embedded, decode_mode='single_step', target=None, teacher_forcing_ratio=0.5, tgt_key_padding_mask=None):
        if decode_mode == 'autoregressive':
            return self.autoregressive_decode(memory, subject_embedded, target, teacher_forcing_ratio)
        elif decode_mode == 'single_step':
            return self.single_step_decode(memory, subject_embedded, tgt_key_padding_mask)
        else:
